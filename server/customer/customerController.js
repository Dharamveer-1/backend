...
register = (req, res) => {
  ...
  if (!req.body.contact) validationerror.push("contact is required");
  if (!req.body.address) validationerror.push("address is required");
    ...
    User.findOne({ email: req.body.email })
      .then((userData) => {
        if (!userData) {
          ...
          userObj.save().then((saveCustomer) => {
